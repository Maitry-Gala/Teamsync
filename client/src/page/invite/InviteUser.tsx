...
const InviteUser = () => {
  ...
  const { data: authData, isLoading: isPending } = useAuth();
  const user = authData?.user;
  console.log("isPending:", isPending, "user:", authData?.user, "token:", useStore.getState().accessToken, "hydrated:", useStore.getState()._hasHydrated);
  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    ...
    mutate(inviteCode, {
      onSuccess: async (data) => {
        ...
        await queryClient.invalidateQueries({ queryKey: ["workspace", data.workspaceId] });
